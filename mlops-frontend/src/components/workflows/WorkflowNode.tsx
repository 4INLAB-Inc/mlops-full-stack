...
interface WorkflowNodeProps extends NodeProps {
  onNodeAction: (action: 'run' | 'stop' | 'delete', nodeId: string) => void;
}

const WorkflowNode = ({ data, id, isConnectable, onNodeAction }: WorkflowNodeProps) => {
  const bgColor = useColorModeValue('white', 'gray.800')
  const borderColor = useColorModeValue('gray.200', 'gray.600')
  const shadowColor = useColorModeValue('rgba(0, 0, 0, 0.1)', 'rgba(0, 0, 0, 0.4)')
  ...
  const statusConfig = (() => {
    switch (data.status) {
      case 'running': return { color: 'orange.400', icon: FiLoader, text: '실행 중', animation: true }
      case 'completed': return { color: 'green.400', icon: FiCheck, text: '완료', animation: false }
      case 'error': return { color: 'red.400', icon: FiX, text: '오류', animation: false }
      default: return { color: 'gray.400', icon: FiClock, text: '대기 중', animation: false }
    }
  })()
  ...
  const handleAction = (action: 'run' | 'stop' | 'delete') => {
    if (typeof onNodeAction === 'function') {
      onNodeAction(action, id)  // ✅ Đúng: dùng prop onNodeAction
    }
  }
  return (
    <MotionBox
      ...
      bg={bgColor}
      border="1px solid"
      borderColor={data.selected ? statusConfig.color : borderColor}
      borderRadius="xl"
      boxShadow={`0 4px 12px ${shadowColor}`}
      p={4}
      width="280px"
    ...
    >
      ...
      <VStack spacing={3} align="stretch">
        ...
        {data.parameters && (
          <Box>
            <Text fontSize="sm" fontWeight="medium" mb={1}>파라미터</Text>
            <VStack align="stretch" spacing={1}>
              {Object.entries(data.parameters).map(([key, value]) => (
                <HStack key={key} fontSize="xs" justify="space-between">
                  <Text color="gray.500">{key}</Text>
